...
def prediction_step(sess, dataset, dataset_type, model, transition_params_trained, stats_graph_folder,
    ...
    if dataset_type != 'deploy':
        if main_evaluation_mode == 'conll':
            ...
            new_y_pred, new_y_true, new_label_indices, new_label_names, _, _ = remap_labels(all_predictions, all_y_true, dataset, main_evaluation_mode)
...
def predict_labels(sess, model, transition_params_trained, dataset, epoch_number, stats_graph_folder, dataset_filepaths,tagging_format,main_evaluation_mode,
                    use_crf=True):
...
def prediction_step_lite(sess, dataset, dataset_type, model, transition_params_trained, stats_graph_folder,
    ...
    #                                                                                 dataset, main_evaluation_mode)
    # class_name = [ dataset.index_to_label[i] for i in len(dataset.index_to_label.keys())]
    new_y_pred=[]
    new_y_true=[]
